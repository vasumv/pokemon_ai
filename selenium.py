...
class Selenium():
    ...
    def __init__(self, agent, alive, url=BASE_URL, driver_path="/home/vasu/Downloads/chromedriver"):
        self.agent = agent
        self.url = url
        self.driver_path = driver_path
        self.alive = alive
    def move(self, index, backup_switch):
        move = driver.find_element_by_xpath("/html/body/div[4]/div[5]/div/div[2]/div[2]/button[%d]" % (index + 1))
        move.click()
        if not self.alive:
            backup_switch(backup_switch)
    def switch(self, index, backup_switch):
        choose = driver.find_element_by_xpath("/html/body/div[4]/div[5]/div/div[2]/div[2]/button[%d]" % (index + 1))
        choose.click()
        if not self.alive:
            backup_switch(backup_switch)
    ...
    def get_opp_team():
        with open("data/poke.json") as f:
            ...
            names = driver.find_element_by_xpath("/html/body/div[4]/div[3]/div[1]/div[15]/em")
            ...
            for i in range(2, 4):
                for j in range(1, 4):
                    opp_health = driver.find_element_by_xpath("/html/body/div[4]/div[1]/div/div[8]/div/div[%d]/span[%d]" % (i, j))
            ...
            for name in name_list:
                info = poke_dict[name]
                poke = Pokemon(name, info[name].typing, info[name].stats, info[name].movesets[0])
